...
class PartCATHKGStage1(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        part_logits = F.interpolate(part_logits_low, size=x.shape[-2:], mode="bilinear", align_corners=False)
        role_logits = F.interpolate(role_logits_low, size=x.shape[-2:], mode="bilinear", align_corners=False)
        support_logits = F.interpolate(support_logits_low, size=x.shape[-2:], mode="bilinear", align_corners=False)
