...
def build_aog_hkg(stage1_model, loader, schema: RoleSchema, cfg: HKGConfig, *, device: str = "cuda") -> AOGHierarchicalKG:
    ...
    for batch in loader:
        ...
        for b in range(images.shape[0]):
            ...
            for k in active_parts:
                ...
                count_cf[c, k] += 1
                cnt_func[k] += 1
                sum_func_r[k] += func_r[b, k]
                sum_func_d[k] += func_d[b, k]
                ...
                if rid >= 0 and bool(pred_role_presence[b, rid].item() > 0.5):
                    ...
                    sum_class_r[c, k] += role_r[k]
                    sum_class_d[c, k] += role_d[k]
    ...
    for idx, rec in enumerate(records):
        ...
        for k in active:
            t_count[c, a, k] += 1
            t_sum_r[c, a, k] += rec.role_r[k]
            t_sum_d[c, a, k] += rec.role_d[k]
    ...
    t_proto_r = _safe_norm(t_sum_r / t_count.clamp_min(1).unsqueeze(-1))
    t_proto_d = _safe_norm(t_sum_d / t_count.clamp_min(1).unsqueeze(-1))
    t_proto_r = _fallback_proto(t_proto_r, t_count, class_role_proto_r, func_proto_r)
    t_proto_d = _fallback_proto(t_proto_d, t_count, class_role_proto_d, func_proto_d)
