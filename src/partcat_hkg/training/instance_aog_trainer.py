...
@torch.no_grad()
def evaluate_instance_aog_stage2(model, loader, cfg: ProjectConfig, *, device: str = "cuda", enable_edges: bool = True, max_batches: int | None = None) -> dict[str, float]:
    model.eval()
    ...
    row.update({
        ...
        "val_isaog_acc": accuracy(hkg_correct, total),
    })
    return row
...
def train_instance_aog_stage2(model, train_loader, val_loader, cfg: ProjectConfig, *, device: str = "cuda") -> list[dict[str, float]]:
    model.to(device)
    model.freeze_stage1()
    ...
    for epoch in range(1, int(cfg.training.stage2_epochs) + 1):
        ...
        for batch in train_loader:
            ...
            if not torch.isfinite(loss):
                raise FloatingPointError(f"Instance-AOG Stage2 non-finite loss at epoch {epoch}: {float(loss.detach().cpu())}")
        ...
        if val_loader is not None:
            row.update(evaluate_instance_aog_stage2(model, val_loader, cfg, device=device, enable_edges=True))
        history.append(row)
        save_json(Path(cfg.paths.save_dir) / "stage2_instance_aog_history.json", history)
        _save_history_csv(Path(cfg.paths.save_dir) / "stage2_instance_aog_history.csv", history)
        ...
        save_checkpoint(ckpt_dir / "stage2_instance_aog_last.pt", model, extra=extra)
        if score >= best:
            best = score
            save_checkpoint(ckpt_dir / "stage2_instance_aog_best.pt", model, extra=extra)
        print(
            f"[stage2-instance-aog] epoch={epoch} "
            ...
            f"val_isaog={row.get('val_isaog_acc', float('nan')):.4f} "
