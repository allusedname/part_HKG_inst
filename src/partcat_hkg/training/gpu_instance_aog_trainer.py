...
@torch.no_grad()
def evaluate_gpu_instance_aog_stage2(model, loader, cfg: ProjectConfig, *, device: str = "cuda", enable_edges: bool = True, max_batches: int | None = None) -> dict[str, float]:
    model.eval()
...
def train_gpu_instance_aog_stage2(model, train_loader, val_loader, cfg: ProjectConfig, *, device: str = "cuda") -> list[dict[str, float]]:
    model.to(device)
    ...
    for epoch in range(1, int(cfg.training.stage2_epochs) + 1):
        ...
        for batch in train_loader:
            ...
            if not torch.isfinite(loss):
                raise FloatingPointError(f"GPU IS-AOG Stage2 non-finite loss at epoch {epoch}: {float(loss.detach().cpu())}")
        ...
        if val_loader is not None:
            row.update(evaluate_gpu_instance_aog_stage2(model, val_loader, cfg, device=device, enable_edges=True))
        history.append(row)
        save_json(Path(cfg.paths.save_dir) / "stage2_gpu_instance_aog_history.json", history)
        _save_history_csv(Path(cfg.paths.save_dir) / "stage2_gpu_instance_aog_history.csv", history)
        ...
        save_checkpoint(ckpt_dir / "stage2_gpu_instance_aog_last.pt", model, extra=extra)
        if score >= best:
            best = score
            save_checkpoint(ckpt_dir / "stage2_gpu_instance_aog_best.pt", model, extra=extra)
        print(
            f"[stage2-gpu-isaog] epoch={epoch} "
