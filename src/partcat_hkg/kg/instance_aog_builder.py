...
def build_instance_aog(stage1_model, loader, schema: RoleSchema, cfg: Any, *, device: str = "cuda") -> InstanceAOG:
    ...
    Repeated functional parts are treated as unordered connected components.
    Template-local latent slots are learned by clustering component layouts, and
    observed components are assigned to slots only inside a parse hypothesis.
    """
    ...
    for c in range(cnum):
        for a in range(anum):
            ...
            for k in range(fnum):
                ...
                for _center in ordered_centers:
    ...
    family_centers: dict[int, list[tuple[int, list[tuple[torch.Tensor, torch.Tensor]]]]] = defaultdict(list)
